...
from sprox.fillerbase import ModelsFiller, ModelDefFiller, EditFormFiller, AddFormFiller, FormFiller, TableFiller
...
class ConfigCache(object):
    ...
    def __init__(self, session, metadata=None):
        ...
        self._provider = SAORMSelector.get_provider(
            hint=session.bind, metadata=self.metadata)
    ...
    def _get(self, key):
        ...
        if view_type not in self.default_configs:
            raise ConfigCacheError(
                'view_type:%s not found in default bases' % view_type)
